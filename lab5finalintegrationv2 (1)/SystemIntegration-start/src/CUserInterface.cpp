...
void CUserInterface::init(PLAYER_CV_DEVS playerCtrlDev) {
	if (playerCtrlDev == CONSOLE) {
		...
		printMessage(" Using IOWarrior control. ");
	}
	...
	//m_ampMeter.init(CPlayerCVDevice *pIoDev, SCALING_MODE scmode, float inValMin, float inValMax, int8_t outLogValMin = 0);
...
void CUserInterface::setAmplitudeScaling(CAmpMeter::SCALING_MODE mode) {
	...
	m_ampMeter.init(m_playerCVDev, mode, -2, 2, outLogValMin);
